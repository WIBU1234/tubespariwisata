...
import 'package:tubespariwisata/page/homepage/detailPageElement/destinasiWisata.dart';
...
void pushForgotPasswordNew(BuildContext context, User user) {
  Navigator.push(
      context, MaterialPageRoute(builder: (_) => NewPasswordPage(user: user)));
}

void pushUpdateProfile(BuildContext context, User user) {
  Navigator.push(
      context, MaterialPageRoute(builder: (_) => const UpdatePage()));
}

void pushCameraPage(BuildContext context, User user) {
  Navigator.push(context,
      MaterialPageRoute(builder: (_) => CameraPage(cameras: null, user: user)));
}

void pushScanQr(BuildContext context) {
  Navigator.push(context,
      MaterialPageRoute(builder: (_) => const BarcodeScannerPageView()));
}

void ticketPage(BuildContext context, Destinasi destinasi) {
  Navigator.push(context,
      MaterialPageRoute(builder: (_) => DestinasiWisata(destinasi: destinasi)));
}
...
void updateDestinasi(BuildContext context, Destinasi destinasi) {
  Navigator.push(
      context,
      MaterialPageRoute(
          builder: (_) => UpdatePageDestination(destinasi: destinasi)));
}
...
class LabelTextConstant {
  ...
  static const String scanQrPlaceHolderLabel =
      "Scanf something & click to copy to clipboard";
